...
@interface CFFindController ()<UITableViewDataSource ,UITableViewDelegate ,UICollectionViewDelegateFlowLayout ,UICollectionViewDataSource ,UICollectionViewDelegate>
...
@implementation CFFindController
...
- (UICollectionView *)rightCollectionView
{
    if (!_rightCollectionView) {
        ...
        [_rightCollectionView registerClass:[CFRightReusableView class] forSupplementaryViewOfKind:UICollectionElementKindSectionHeader withReuseIdentifier:RightReusableViewID];
    }
...
- (void)loadSortData
{
    ...
    [CFNetworkTools postResultWithUrl:Classification_Interface params:params success:^(id  _Nonnull responseObject) {
        CFLog(@"--->%@",responseObject);
        [TFProgressHUD dismiss];
        homeSelf.leftCategory = [CFLeftCategory mj_objectArrayWithKeyValuesArray:responseObject[@"trade_one"]];
        [self setupMenuView];
...
- (void)setupMenuView
{
    _rightArray = [[self.leftCategory objectAtIndex:0] valueForKey:@"trade_two"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    _rightArray = [[self.leftCategory objectAtIndex:indexPath.row] valueForKey:@"trade_two"];
...
{
    NSArray *array = [[_rightArray objectAtIndex:section] valueForKey:@"trade_three"];
    return [array count];
}
...
- (UICollectionViewCell *)collectionView:(UICollectionView *)collectionView cellForItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSArray *array = [[_rightArray objectAtIndex:indexPath.section] valueForKey:@"trade_three"];
    self.rightCategory = [CFRightCategory mj_objectArrayWithKeyValuesArray:array];
...
{
    CFRightReusableView *view =  [collectionView dequeueReusableSupplementaryViewOfKind:kind   withReuseIdentifier:RightReusableViewID forIndexPath:indexPath];
    
    NSString *title = [[_rightArray objectAtIndex:indexPath.section] valueForKey:@"TRADE_NAME"];
...
{
    CGSize size = {CFMainScreen_Width , 40};
    return size;
}
...
- (void)collectionView:(UICollectionView *)collectionView didSelectItemAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (self.from) {
    self.changeIndustryBlock(self.rightCategory[indexPath.row].TRADE_ID ,self.rightCategory[indexPath.row].TRADE_NAME);
        
        [self.navigationController popViewControllerAnimated:YES];
    } else {
        CFMenuTypeController *sortListVC = [[CFMenuTypeController alloc] init];
        sortListVC.navigationItem.title = @"行业人脉";
        sortListVC.industry_id = self.rightCategory[indexPath.row].TRADE_ID;
