...
static TFPopupViewStyle _style;           // 遮罩类型
static TFPopupViewShowStyle _showStyle;       // 显示类型
static TFPopupViewAnimStyle _animStyle;       // 动画类型
static BOOL _hasCover;               // 遮罩是否已经显示的判断值
static BOOL _isHideStatusBar;        // 遮罩是否遮盖状态栏

// 分离动画类型
static TFPopupViewShowAnimStyle _showAnimStyle;
static TFPopupViewHideAnimStyle _hideAnimStyle;
...
+ (instancetype)transparentCoverWithTarget:(id)target action:(SEL)action
{
    ...
    bgView.xtfei_size = CGSizeMake(TFMainScreen_Width, TFMainScreen_Height);
...
+ (UIView *)transparentBgView
{
    UIView *bgView = [UIView new];
    bgView.backgroundColor = [UIColor clearColor];
    bgView.xtfei_size = _cover.xtfei_size;
    bgView.userInteractionEnabled = YES;
    if (!_notclick) {
...
{
    if (_animated) {
        _contentView.xtfei_y = TFMainScreen_Height;
        [UIView animateWithDuration:0.25 animations:^{
            _contentView.xtfei_y = TFMainScreen_Height - _contentView.xtfei_height;
...
// 常见遮罩
+ (void)topCover:(UIView *)fromView contentView:(UIView *)contentView style:(TFPopupViewStyle)style notClick:(BOOL)notClick animated:(BOOL)animated
{
    TFPopupViewAnimStyle animStyle;
    if (animated) {
        animStyle = TFPopupViewAnimStyleTop;
    } else {
        animStyle = TFPopupViewAnimStyleNone;
    }
    
    [self coverFrom:fromView contentView:contentView style:style showStyle:TFPopupViewShowStyleTop animStyle:animStyle notClick:notClick];
}

+ (void)bottomCoverFrom:(UIView *)fromView contentView:(UIView *)contentView style:(TFPopupViewStyle)style notClick:(BOOL)notClick animated:(BOOL)animated
{
    TFPopupViewAnimStyle animStyle;
    if (animated) {
        animStyle = TFPopupViewAnimStyleBottom;
    } else {
        animStyle = TFPopupViewAnimStyleNone;
    }
    
    [self coverFrom:fromView contentView:contentView style:style showStyle:TFPopupViewShowStyleBottom animStyle:animStyle notClick:notClick];
}

+ (void)centerCover:(UIView *)contentView style:(TFPopupViewStyle)style notClick:(BOOL)notClick animated:(BOOL)animated
{
    TFPopupViewAnimStyle animStyle;
    if (animated) {
        animStyle = TFPopupViewAnimStyleCenter;
    } else {
        animStyle = TFPopupViewAnimStyleNone;
    }
    
    [self coverFrom:[self getKeyWindow] contentView:contentView style:style showStyle:TFPopupViewShowStyleCenter animStyle:animStyle notClick:notClick];
}
 ...
 */
+ (void)coverFrom:(UIView *)fromView contentView:(UIView *)contentView style:(TFPopupViewStyle)style showStyle:(TFPopupViewShowStyle)showStyle animStyle:(TFPopupViewAnimStyle)animStyle notClick:(BOOL)notClick
{
...
+ (void)coverFrom:(UIView *)fromView contentView:(UIView *)contentView style:(TFPopupViewStyle)style showStyle:(TFPopupViewShowStyle)showStyle animStyle:(TFPopupViewAnimStyle)animStyle notClick:(BOOL)notClick showBlock:(showBlock)showBlock hideBlock:(hideBlock)hideBlock
{
    if ([self hasCover]) return;
    ...
    if (style == TFPopupViewStyleTranslucent) { // 半透明
        cover.backgroundColor = [UIColor blackColor];
        cover.alpha = kAlpha;
        [self addTap:cover];
    } else if (style == TFPopupViewStyleTransparent) {  // 全透明
        cover.backgroundColor = [UIColor clearColor];
        [cover addSubview:[self xtfei_transparentBgView]];
    } else { // 毛玻璃，高斯模糊
        cover.backgroundColor = [UIColor clearColor];
        [self addTap:cover];
        // 添加高斯模糊效果,添加毛玻璃效果
 ...
 *  透明图片
 */
+ (UIView *)xtfei_transparentBgView
{
    UIView *bgView = [UIView new];
    bgView.backgroundColor = [UIColor clearColor];
    bgView.xtfei_size = _cover.xtfei_size;
...
+ (UIView *)xtfei_coverTransparentBgView
{
    UIView *bgView = [UIView new];
    bgView.backgroundColor = [UIColor clearColor];
    bgView.xtfei_size = _cover.xtfei_size;
...
{
    [_fromView addSubview:_contentView];
    _contentView.xtfei_centerX = _fromView.xtfei_centerX;
    
    if (_showStyle == TFPopupViewShowStyleTop) {
        if (_animStyle == TFPopupViewAnimStyleTop) {
            _contentView.xtfei_y = -_contentView.xtfei_height;
            [UIView animateWithDuration:kAnimDuration animations:^{
                _contentView.xtfei_y = 0;
            }completion:^(BOOL finished) {
                !_showBlock ? : _showBlock();
            }];
        } else {
    ...
    } else if (_showStyle == TFPopupViewShowStyleCenter){
        if (_animStyle == TFPopupViewAnimStyleTop) { // 上进下出
            _contentView.xtfei_y = -_contentView.xtfei_height;
            [UIView animateWithDuration:kAnimDuration animations:^{
                _contentView.center = _fromView.center;
            }completion:^(BOOL finished) {
        ...
        } else if (_animStyle == TFPopupViewAnimStyleCenter) { // 中间动画
            _contentView.center = _fromView.center;
            [self animationAlert:_contentView];
        } else if (_animStyle == TFPopupViewAnimStyleBottom) { // 下进上出
            _contentView.xtfei_y = TFMainScreen_Height;
            [UIView animateWithDuration:kAnimDuration animations:^{
                _contentView.center = _fromView.center;
            }completion:^(BOOL finished) {
    ...
    } else if (_showStyle == TFPopupViewShowStyleBottom){
        if (_animStyle == TFPopupViewAnimStyleBottom) {
            _contentView.xtfei_y = TFMainScreen_Height;
            [UIView animateWithDuration:0.25 animations:^{
                _contentView.xtfei_y = TFMainScreen_Height - _contentView.xtfei_height;
            }completion:^(BOOL finished) {
...
+ (void)hideView
{
    ...
    if (_showStyle == TFPopupViewShowStyleTop) {
        if (_animStyle == TFPopupViewAnimStyleTop) { // 上进上出
            [UIView animateWithDuration:kAnimDuration animations:^{
                _contentView.xtfei_y = -_contentView.xtfei_height;
            }completion:^(BOOL finished) {
                [self remove];
        ...
        if (_animStyle == TFPopupViewAnimStyleBottom) {  // 下进下出
            [UIView animateWithDuration:0.25 animations:^{
                _contentView.xtfei_y = TFMainScreen_Height;
            }completion:^(BOOL finished) {
                [self remove];
...
+ (void)coverFrom:(UIView *)fromView contentView:(UIView *)contentView style:(TFPopupViewStyle)style showStyle:(TFPopupViewShowStyle)showStyle showAnimStyle:(TFPopupViewShowAnimStyle)showAnimStyle hideAnimStyle:(TFPopupViewHideAnimStyle)hideAnimStyle notClick:(BOOL)notClick
{
...
+ (void)coverFrom:(UIView *)fromView contentView:(UIView *)contentView style:(TFPopupViewStyle)style showStyle:(TFPopupViewShowStyle)showStyle showAnimStyle:(TFPopupViewShowAnimStyle)showAnimStyle hideAnimStyle:(TFPopupViewHideAnimStyle)hideAnimStyle notClick:(BOOL)notClick showBlock:(showBlock)showBlock hideBlock:(hideBlock)hideBlock
{
    if ([self hasCover]) return;
    ...
    switch (style) {
        case TFPopupViewStyleTranslucent: // 半透明
            [self setupTranslucentCover:cover];
            break;
        case TFPopupViewStyleTransparent: // 全透明
            [self setupTransparentCover:cover];
            break;
        case TFPopupViewStyleBlur:        // 高斯模糊
...
+ (void)coverHideStatusBarWithContentView:(UIView *)contentView style:(TFPopupViewStyle)style showStyle:(TFPopupViewShowStyle)showStyle showAnimStyle:(TFPopupViewShowAnimStyle)showAnimStyle hideAnimStyle:(TFPopupViewHideAnimStyle)hideAnimStyle notClick:(BOOL)notClick showBlock:(showBlock)showBlock hideBlock:(hideBlock)hideBlock {
...
+ (void)showCover
{
    ...
    switch (_showStyle) {
        case TFPopupViewShowStyleTop:  // 显示在顶部
        {
            if (_showAnimStyle == TFPopupViewShowAnimStyleTop) {
                _contentView.xtfei_y = -_contentView.xtfei_height;
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.xtfei_y = 0;
                }completion:^(BOOL finished) {
            ...
            break;
        case TFPopupViewShowStyleCenter: // 显示在中间
        {
            if (_showAnimStyle == TFPopupViewShowAnimStyleTop) { // 上进
                _contentView.xtfei_y = -_contentView.xtfei_height;
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.center = _fromView.center;
                }completion:^(BOOL finished) {
            ...
            } else if (_showAnimStyle == TFPopupViewShowAnimStyleCenter) { // 中间动画
                _contentView.center = _fromView.center;
                [self animationAlert:_contentView];
            } else if (_showAnimStyle == TFPopupViewShowAnimStyleBottom) { // 下进
                _contentView.xtfei_y = TFMainScreen_Height;
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.center = _fromView.center;
                }completion:^(BOOL finished) {
            ...
            break;
        case TFPopupViewShowStyleBottom:  // 显示在底部
        {
            if (_showAnimStyle == TFPopupViewShowAnimStyleBottom) {
                _contentView.xtfei_y = TFMainScreen_Height;
                [UIView animateWithDuration:0.25 animations:^{
                    _contentView.xtfei_y = TFMainScreen_Height - _contentView.xtfei_height;
                }completion:^(BOOL finished) {
...
+ (void)hideCover
{
    ...
    switch (_showStyle) {
        case TFPopupViewShowStyleTop:  // 显示在顶部
        {
            if (_hideAnimStyle == TFPopupViewHideAnimStyleTop) {
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.xtfei_y = -_contentView.xtfei_height;
                }completion:^(BOOL finished) {
                    [self remove];
        ...
        case TFPopupViewShowStyleCenter:  // 显示在中间
        {
            if (_hideAnimStyle == TFPopupViewHideAnimStyleTop) { // 上出
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.xtfei_y = -_contentView.xtfei_height;
                }completion:^(BOOL finished) {
                    [self remove];
                }];
            } else if (_hideAnimStyle == TFPopupViewHideAnimStyleCenter) { // 中间动画
                [self remove];
            } else if (_hideAnimStyle == TFPopupViewHideAnimStyleBottom) { // 下出
                [UIView animateWithDuration:kAnimDuration animations:^{
                    _contentView.xtfei_y = TFMainScreen_Height;
                }completion:^(BOOL finished) {
                    [self remove];
        ...
        case TFPopupViewShowStyleBottom:  // 显示在底部
        {
            if (_hideAnimStyle == TFPopupViewHideAnimStyleBottom) {
                [UIView animateWithDuration:0.25 animations:^{
                    _contentView.xtfei_y = TFMainScreen_Height;
                }completion:^(BOOL finished) {
                    [self remove];
...
{
    cover.backgroundColor = [UIColor clearColor];
    [cover addSubview:[self xtfei_coverTransparentBgView]];
}
...
+ (void)layoutSubViews
{
    ...
    switch (_showStyle) {
        case TFPopupViewShowStyleTop:
        {
            _contentView.xtfei_y = 0;
        }
            break;
        case TFPopupViewShowStyleCenter:
        {
            _contentView.center = _fromView.center;
        }
            break;
        case TFPopupViewShowStyleBottom:
        {
            _contentView.xtfei_y = TFMainScreen_Height - _contentView.xtfei_height;
        }
            break;
